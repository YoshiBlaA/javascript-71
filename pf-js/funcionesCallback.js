...
numerosXD.forEach((numero, index, array) => {
    console.log(array);
});
...
const numerosMasDos = numeros.forEach((numero, index, array) => {
...
/**
 * Map ejecuta una funcion sobre cada elemento de un array y devuelve un nuevo array con el resutlado.
 */
function multiplyBy3(numero) {
    return numero * 3;
}

const numersXTres = numeros.map(number => multiplyBy3(number));
console.log(numersXTres);
...
const multiplosDeTres = numeros.filter(function (numero) {
    return numero % 3 == 0
});
console.log(multiplosDeTres);   

// Agregar iva a cada elemento del array de numeros usando map pero con una funcion externa

function anniadirIVA(numero){
    return numero * 1.16;
}

const numerosConIVA = numeros.map(numero => anniadirIVA(numero));
console.log("Numeros de la lista con un IVA agregado");
